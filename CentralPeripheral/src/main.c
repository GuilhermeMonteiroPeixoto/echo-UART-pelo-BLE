						 ...
						 struct net_buf_simple *ad)
{
	...
	// Tenta parar a varredura BLE antes de iniciar uma conexão.
	// Se não conseguir parar a varredura, retorna sem tentar conectar.
	if (bt_le_scan_stop())
	{
		return;
	}
...
void main(void)
{
	...
	// Inicializa a pilha Bluetooth. Se um callback for necessário após a inicialização,
	// pode-se substituir NULL por uma função de callback adequada.
	err = bt_enable(NULL);
	// Verifica se houve erro na inicialização da pilha Bluetooth.
	if (err)
	{
		printk("Bluetooth init failed (err %d)\n", err);
		// Encerra a função main se houver falha na inicialização.
		return;
	}

	printk("Bluetooth initialized\n");

	// Inicia a varredura por dispositivos BLE periféricos que estão anunciando.
	start_scan();
	// Inicializa a leitura de entrada do usuário pelo console,
	// permitindo interação com o sistema.
	read_entry();
}